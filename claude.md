...
- Walk leg: for a mapped building origin, `walkMinutes()` looks up the precomputed Mapbox walking pair in `src/data/walk_matrix.json` first (`WALK_MATRIX.bldgs[bldgNum][stopName]`); on miss it falls through to `haversine(origin, stop)` divided by 5 km/h, floored at 3 min. Origin is the user's geolocation if the "Nearest stop" button was used, else the picked building's OSM centroid (`src/data/buildings_osm.json`), else the 3-min mock. No geolocation request on page load — only on explicit button click. The matrix ships in the PWA bundle, is regenerated by `scripts/gen_walk_matrix.py` (source-hash gated on `stop_coords.json` + `buildings_osm.json`), and only carries each building's nearest stop — other bldg→stop pairs are matrix misses and use haversine. For geolocation origins, Phase 3 shipped the runtime path: `App.jsx search()` prefetches through `prefetchUserWalks()` (see `src/lib/walk-runtime.js`), which calls `/api/walk` on the Cloudflare Worker (`worker/index.js`, backed by the `MAPBOX_TOKEN` secret), coord-rounds the origin to a ~30 m grid, and caches results in `localStorage` under a `walk_matrix._meta.source_hash`-prefixed key so refreshing the matrix invalidates the cache atomically. Any network failure, non-2xx, or `meters > 2× haversine` sanity trip → the map override is absent for that stop → `walkMinutes` falls through to haversine. UI still prefixes matrix and geolocation walks with `~` (Phase 4 will drop that for `source: "mapbox"` walks, matching the `pdf` vs `heuristic` pattern on departures).

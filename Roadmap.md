...
- **QR-code posters + bus-stop stickers** at Bus Terminal, Pedestrian Gate, individual stop shelters, chapels, DFACs (Provider Grill, Spartan, Pittman, Talon), Sitman + Collier gyms. Highest-signal channel because it hits users at the moment of need. Design in EN + KO on one poster. QR generated by `scripts/gen_qr.sh` (or `npm run gen:qr`) → `public/qr/humphreysbus-app.svg` + `.png`, error-correction level H (~30% recovery, survives creases and partial coverage). Facility-owned surfaces (chapel bulletin boards, DFAC entrances, gym lobbies) need facility-owner permission per location; stop-shelter stickers ride on the shuttle network itself and go up as informational rider aid alongside the shelter's existing S-#### plate. Nagan suggested BOSS as an additional outreach channel 2026-09-21.
...
- ~~Gold stop-name verification: 7 Exhibit #0019 stops pending~~ **Resolved 2026-08-31, CAC entry re-resolved 2026-09-21.** `Sentry Village PX` → `Sentry Village Shoppette` (AAFES). `CAC` initially mis-identified as Bldg 6400 (LTG Maude Hall / ID Cards / One Stop) — but Bldg 6400 is ~2.8 km from BT, too far for the +3-min offset. Set to placeholder `CAC (Sentry Village)` on 2026-08-31; renamed 2026-09-21 to `Sentry Village Gate` after confirming the Gold poster "CAC" refers to **Central Access Control** (the north entry gate to Sentry Village near OSM lift_gate 36.9490/127.0273), not the Common Access Card (ID) office. The CAC card / DEERS / DBIDS office is at LTG Maude Hall (Bldg 6400), served by Blue/Green/Purple, and now carries the `"CAC"`, `"CAC Processing"`, `"CAC Office"` aliases; the gate stop carries only Sentry Village entry aliases (no CAC-adjacent phrase points at the gate, to eliminate the confusion at the source). Remaining five stops landed via walkthrough + `scripts/apply_walkthrough.py`: `USO` → `USO Sentry Village` (outbound S103) + `USO Sentry Village (Opposite)` (return S376) as separate shelters; `Family Housing Twr 1` → existing `Family Housing Towers (Tropic Lightning Ave)` outbound + new `Family Housing Towers (5050s Block)` return; `Family Housing Twr 2` → existing `Family Housing Towers (Taro Ave)` (both platforms); `Family Housing Twr 3` → new `Family Housing Towers (5100s Block)`; `O-6 Housing` → new `Officer Housing`. All 19 Gold stops now PDF-sourced across Mon–Fri / Sat / Sun. Coordinates for the 5 new stops still pending OSM lookup
